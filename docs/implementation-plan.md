...
Выполнено (Phase 2.7 — Tab Completion):

- реализован метод `getCompletion` в `QalcBridge`, использующий базу юнитов, функций и переменных `libqalculate`;
- добавлена поддержка терминального автодополнения (поиск наиболее длинного общего префикса);
- реализован перехват клавиши Tab в `EditorPane.qml` для активации дополнения под курсором;
- добавлены тесты, подтверждающие наличие привязок автодополнения;
- `npm test` проходит (55/55), CMake build завершен успешно.

Выполнено (Phase 2.8 — Comprehensive Fixes & Logic Polish):

- исправлена ошибка `20/12 = 2`: теперь вычисления всегда идут в режиме float с учётом настройки знаков после запятой;
- переменные теперь сохраняют *результат* вычисления (например, `A = 10 usd to eur` сохранит курс, а не саму строку);
- команда `/help` теперь заменяет текст в основном редакторе на подробную инструкцию;
- исправлено окно настроек: устранены ошибки доступа к свойствам, добавлен выбор шрифта через ComboBox;
- реализована подсветка комментариев через `#` (ярко-жёлтый цвет);
- добавлена поддержка регистронезависимого ввода валют (`usd` -> `USD`);
- исправлена работа автодополнения (Tab) и панели истории ( Drawer z-index);
- `npm test` проходит (55/55), CMake build завершен успешно.

## Финализация проекта

Приложение `numi-kde` теперь является полностью нативным C++/Qt клиентом с мощным математическим движком `libqalculate`. Основные цели портирования достигнуты:
- Нулевая зависимость от Node.js в runtime.
- Поддержка валют, сложных единиц измерения и дат.
- Нативная интеграция с KDE Plasma (Wayland/X11).
