...
- проверка: `npm test` проходит, 40/40.

Следующий шаг:

- расширить extension API diagnostics/loading policy или перейти к первому KDE editor spike.
...
- Next: extension loading policy, more compatibility fixtures, KDE editor spike.
...
1. Decide extension loading policy: manifest shape, allowed directories, diagnostics.
2. Add more date/time fixtures only after deciding how to handle `today` and timezone policy.
3. Add first KDE editor spike with source editor, result column and shared core adapter.
